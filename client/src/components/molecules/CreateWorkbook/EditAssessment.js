...
export default function EditAssessment({ item, close }) {
    ...
    item.questions.map((e, i) => {
      ...
      if (e.type === "checkbox" || e.type === "radio") {
        ...
        e.options.map((option, j) => {
          ...
          optArr.push({ value: "item", key: oKey })
        })
        eleArr.push({
          type: e.type,
          options: optArr,
  ...
  const [initialValues, setInitialValues] = useState(() => {
    ...
    item.questions.map((e, index) => {
      initValues[`${e.type}${index}`] = e.title
      if (e.type === "checkbox" || e.type === "radio") {
        e.options.map((option, j) => {
          initValues[`options${index}${j}`] = option.title
          initValues[`points${index}${j}`] = parseInt(option.points)
        })
      } else {
        initValues[`points${index}`] = parseInt(e.points)
      }
    })
